...
const TodoCard: React.FC<TaskCardProps> = ({ todo, handleCheck, deleteTodo }) => {
  return (
    <div className="flex flex-row items-center justify-between w-[75%] max-w-full h-[75px] bg-[#15101C] rounded-[10px] p-4 my-[2%]">
      {todo.status === "completed" ? (
        <span className="text-[#52E058] text-[16px] line-through">{todo.description}</span>
      ) : (
        <span className="text-white text-[16px]">{todo.description}</span>
      )}

      {todo.status === "in progress" && (
        <div className="flex space-x-2">
          <button
            onClick={() => handleCheck(todo.name)}
            className="bg-[#1D1825] p-2 rounded-md hover:bg-[#1f614d]"
          >
            <svg
              width="18"
              height="13"
          ...
          <button
            ...
            className="bg-[#1D1825] p-2 rounded-md hover:bg-[#801e37]"
          >
            <svg
              width="18"
              height="19"
